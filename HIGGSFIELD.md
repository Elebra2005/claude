...
## Ключ через прокси среды

Если ключ хранится не в переменных окружения, а подставляется прокси (в Claude
Code это **API credentials** у облачного окружения: заголовок `Authorization`,
префикс `Key`, значение `<key_id>:<key_secret>`), включите режим, в котором
клиент не шлёт свой заголовок:

```bash
export HIGGSFIELD_AUTH_VIA_PROXY=1
```

Тогда ключа в сессии нет вообще, а запросы уходят авторизованными. В коде это
`HiggsfieldClient(auth_via_proxy=True)`.

## Запуск за корпоративным прокси

Клиент создаёт сессию с `trust_env=True`, поэтому подхватывает `HTTPS_PROXY` и
системный CA. Если в среде включена политика исходящего трафика, в списке
разрешённых должны быть:

* `platform.higgsfield.ai` — сам API;
* хост, с которого отдаются готовые файлы и куда уходит загрузка кадров
  (presigned-ссылка, обычно поддомен `higgsfield.ai` или его CDN) — иначе
  генерация пройдёт, а скачивание упадёт.

Диагностика: `curl -sS "$HTTPS_PROXY/__agentproxy/status"` — в `recentRelayFailures`
видно точное имя заблокированного хоста.
