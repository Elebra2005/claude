...
Каждые 30 минут присылает в личку отчёт по всем кошелькам: сумма, изменение
в $ и % с прошлой проверки и разбивка по монетам. Поддерживаются EVM-кошельки
во всех сетях с DeFi (через Zerion), Solana, Starknet со стейкингом STRK,
сети Cosmos (Keplr) и биржа Bybit. Если сеть не ответила, бот повторяет
запросы и переключается на запасные узлы. Отчёт уходит, когда ответили все.
...
Neutron и Noble бот получает из него сам. Учитываются свободный баланс,
стейкинг, вывод из стейкинга и награды. Для Injective у Keplr другой
ключ, поэтому его адрес `inj1…` нужно указать в `addresses:`. Пулы
ликвидности Osmosis не оцениваются.
